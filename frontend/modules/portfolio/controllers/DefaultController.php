...
{
    public $meta_keywords= 'Галерея, Atmosphera';
    public $meta_description='Галерея | Atmosphera';
    public $meta_title='Галерея | Atmosphera';
    ...
    public function actionView() {
        ...
        $portfolio = Yii::app()->db->createCommand("SELECT n.anounce, n.publication_date, p.title, p.description, p.gallery_id 
                                                  FROM galleryposts AS n
                                                        LEFT JOIN posting AS p ON p.post_id = n.post_id
                                                  WHERE p.is_active = 1 AND p.post_id = :post_id AND n.publication_date <= NOW()"
                                                )->queryRow(true, array(':post_id'=>$postId));
        if (isset($portfolio)) {
            $gallery_id = $portfolio['gallery_id'];
        }
        ...
        if($portfolio !== false) {
            $model = PortfolioPosts::model()->findByPk($postId);
            $this->render('view',array('portfolio'=>$portfolio, 'photos'=>$photos, 'model'=>$model));
        } else {
